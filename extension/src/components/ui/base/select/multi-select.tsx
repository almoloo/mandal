...
const InnerMultiSelect = ({
	...
	const handleTagKeyDown = (
		...
		const isFirstTag =
			comboBoxContext?.selectedItems?.items?.[0]?.id === value;
	...
	const isSelectionEmpty =
		comboBoxContext?.selectedItems?.items?.length === 0;
				...
				comboBoxContext?.selectedItems?.items?.map((value) => (
					...
					>
						...
						<TagCloseX
							...
							onKeyDown={(event) =>
								handleTagKeyDown(event, value.id)
							}
							onPress={() =>
								comboBoxContext.onRemove(new Set([value.id]))
							}
						/>
					</span>
				))}
